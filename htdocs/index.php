...
// --- class 테이블에서 수업 목록 가져오기 (컬럼 제한) ---
$classes = [];
try {
    // teacher_name, room_number, class_duration 컬럼 제외
    // 여기! 테이블 이름을 'class'로 변경했습니다.
    $stmt = $db->prepare("SELECT id, grade, term, name, category1, category2, category3, credit FROM class ORDER BY grade, term, name");
    $stmt->execute();
...
if ($is_logged_in) {
    try {
        // user_timetables와 class를 JOIN하여 필요한 정보만 조회
        // 여기! 테이블 이름을 'class'로 변경했습니다.
