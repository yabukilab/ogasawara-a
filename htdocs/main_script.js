...
function addDropListeners() {
    ...
    timeSlots.forEach(slot => {
        ...
        slot.addEventListener('drop', function(e) {
            ...
            // category2는 현재 사용되지 않지만, 필요하면 여기에 가져올 수 있습니다.
            // const classCategory2 = draggedClass.dataset.category2;

            // 이미 채워진 셀에 또 드롭하는 경우 방지
            ...
            classItemInCell.classList.add('class-item-in-cell'); // 새로운 클래스 추가
            classItemInCell.setAttribute('draggable', true); // 셀 안의 아이템도 드래그 가능하게 할 경우

            // 중요한 변경: 셀의 data-day와 data-period를 직접 읽어와서 classItemInCell에 저장
            ...
            // 드래그된 아이템의 원본을 목록에서 제거 (선택적)
            // draggedClass.remove(); // 필요한 경우 원본 class-item을 수업 목록에서 제거
...
// removeClassFromTimetable 함수도 class-item-in-cell을 대상으로 하도록 약간 수정
            ...
            if (data.status === 'success') {
                data.timetable.forEach(entry => {
                    ...
                    if (targetCell) {
                        ...
                        targetCell.classList.add('filled-primary'); // 셀 자체에 filled-primary 클래스 유지
