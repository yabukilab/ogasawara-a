// PHP에서 전달받은 전역 변수들
// allClasses: 모든 수업 정보 (class_duration 필드 없음)
// userTimetableData: 현재 로그인된 사용자의 시간표 데이터 (class_duration 필드 없음)
// currentUserId: 현재 로그인된 사용자 ID (null일 수 있음)
// isLoggedIn: 로그인 여부 (true/false)
// periodTimes: 교시별 시간 정보
// FIXED_CLASS_DURATION: PHP에서 정의한 수업 지속 시간 (현재는 2로 고정)
...
// const currentSelectedClassDurationSpan = document.getElementById('currentSelectedClassDuration'); // 더 이상 동적으로 업데이트하지 않음
...
function initializePage() {
    ...
    if (userTimetableData) {
        userTimetableData.forEach(item => {
            ...
            // 맵에 저장할 때 class_duration을 FIXED_CLASS_DURATION으로 설정
            userTimetableMap.set(key, { ...item, class_duration: FIXED_CLASS_DURATION });
        });
    }
...
function renderClassesTable() {
    ...
    if (filteredClasses.length === 0) {
        ...
        noResultsRow.innerHTML = `<td colspan="5" style="text-align: center;">検索結果がありません。</td>`; // 컬럼 수 변경 (コマ数 제외)
        ...
        row.innerHTML = `
            ...
            <td>
                <button class="add-button" data-class-id="${classItem.id}"
                        ...
                        data-class-duration="${FIXED_CLASS_DURATION}"> 選択
                </button>
            </td>
        `;
    ...
    document.querySelectorAll('.add-button').forEach(button => {
        button.onclick = (event) => {
            ...
            selectedClass = {
                ...
                duration: FIXED_CLASS_DURATION // 선택된 수업의 duration을 2로 고정
            };
            ...
            // currentSelectedClassDurationSpan.textContent = selectedClass.duration; // 이제 하드코딩된 값 사용
...
function renderTimetable() {
    ...
    const periods = Array.from({ length: 10 }, (_, i) => i + 1); // 1교시부터 10교시 (열의 헤더)
        ...
        while (currentPeriod <= 10) {
            ...
            if (classInfo && classInfo.class_id) {
                ...
                if (classInfo.period === currentPeriod) { // 저장된 period가 현재 currentPeriod와 일치하는지 확인
                    const duration = FIXED_CLASS_DURATION; // 수업 지속 시간 2로 고정
            ...
            if (!rendered) {
                ...
                // 이 부분을 검사하여 이미 병합된 셀에 포함되는 교시는 건너뛰도록 개선
                ...
                for (let p = 1; p < currentPeriod; p++) {
                    ...
                    if (prevClassInfo && prevClassInfo.class_id && p + FIXED_CLASS_DURATION > currentPeriod) {
    ...
    document.querySelectorAll('.time-slot').forEach(timeSlotDiv => {
        ...
        timeSlotDiv.onclick = (event) => {
            ...
            if (selectedClass) {
                ...
                if (isLoggedIn) {
                    addOrUpdateTimetableEntry(selectedClass.id, day, period, currentGrade, FIXED_CLASS_DURATION);
                } else {
                    alert('ログインして時間割を編集してください。');
                }
            } else if (currentClassIdInSlot) {
                // 선택된 수업은 없지만, 칸에 수업이 이미 있다면 해당 수업 정보를 선택된 수업으로 표시
                // (이전과 달리 teacher_name, room_number는 없음)
                ...
                if (existingClass) {
                    selectedClass = {
                        ...
                        duration: FIXED_CLASS_DURATION // duration을 2로 고정
                    };
                    currentSelectedClassNameSpan.textContent = selectedClass.name;
                    currentSelectedClassCreditSpan.textContent = selectedClass.credit;
                    // currentSelectedClassDurationSpan.textContent = selectedClass.duration; // 하드코딩된 값 사용
                    selectedClassInfoDiv.style.display = 'block';
                }
            } else {
...
function addOrUpdateTimetableEntry(classId, startDay, startPeriod, grade, duration) {
    ...
    // 기존 수업이 있다면 모두 삭제 (중복 방지 및 겹치지 않게 하기 위함)
    // 이 로직은 `startPeriod`와 `duration`에 해당하는 모든 교시를 삭제함
    // 하지만 Map에는 수업의 '시작' 교시에만 데이터가 있으므로, 정확히 그 시작 교시만 삭제하고 다시 삽입.
    // Map의 특성상 key가 같으면 덮어쓰기 되므로, delete-then-set 방식이 더 안전.
    for (let p = 1; p <= 10; p++) { // 해당 요일의 모든 교시를 순회하며 삭제할 수업이 있는지 확인
        const key = `${currentUserId}-${grade}-${startDay}-${p}`;
        const entry = userTimetableMap.get(key);
        if (entry && entry.class_id && entry.class_id === selectedClass.id) {
            userTimetableMap.delete(key);
        }
    }
    ...
    userTimetableMap.set(`${currentUserId}-${grade}-${startDay}-${startPeriod}`, {
        ...
        // teacher_name: classToAdd.teacher_name, // 컬럼 없음
        // room_number: classToAdd.room_number,   // 컬럼 없음
        class_duration: duration // 수업 지속 시간도 맵에 저장 (2로 고정)
    });
...
function removeTimetableEntry(day, period, grade) {
    ...
    let durationOfClass = FIXED_CLASS_DURATION; // 2시간 고정
    ...
    for (let p = 1; p <= period; p++) {
        ...
        if (entry && entry.class_id) {
            ...
            if (p + durationOfClass > period) { // 시작 교시 + 지속 시간이 현재 클릭한 교시보다 큰 경우
...
confirmTimetableBtn.addEventListener('click', () => {
    ...
    .then(data => {
        if (data.status === 'success') {
            alert(data.message);
            location.reload(); // 새로고침하여 최신 데이터 반영 (가장 간단한 방법)
        } else {
            alert(data.message);
        }
    })
